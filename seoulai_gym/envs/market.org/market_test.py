...
from seoulai_gym.envs.market.price import Price, Bithumb
from seoulai_gym.envs.market.agents import RandomAgent


@pytest.fixture
def price():
    return Price(Bithumb)


@pytest.fixture
def agent():
    return RandomAgent("test", 100000000)
...
class TestMarket(object):
    ...
    def test_cash_asset_in_action(self, market, agent):
        ...
        obs, rew, done, info = market.step(
            agent, Constants().BUY, cur_price, 1)
        ...
        obs, rew, done, info = market.step(
            agent, Constants().SELL, cur_price, 1)
